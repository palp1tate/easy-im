...
type (
	friendRequestModel interface {
		...
		ListFriendRequest(ctx context.Context, userId string) ([]*FriendRequest, error)
		Update(ctx context.Context, session sqlx.Session, data *FriendRequest) error
...
func (m *defaultFriendRequestModel) Trans(ctx context.Context, fn func(ctx context.Context, session sqlx.Session) error) error {
...
func (m *defaultFriendRequestModel) FindByReqUidAndUserId(ctx context.Context, rid, uid string) (*FriendRequest, error) {
	query := fmt.Sprintf("select %s from %s where `req_uid` = ? and `user_id` = ?", friendRequestRows, m.table)
...
func (m *defaultFriendRequestModel) ListFriendRequest(ctx context.Context, userId string) ([]*FriendRequest, error) {
	query := fmt.Sprintf("select %s from %s where `handle_result` = 1 and `user_id` = ?", friendRequestRows, m.table)
...
func (m *defaultFriendRequestModel) Update(ctx context.Context, session sqlx.Session, data *FriendRequest) error {
	friendRequestIdKey := fmt.Sprintf("%s%v", cacheFriendRequestIdPrefix, data.Id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
